...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kCanniff_Philip_ProjectTwo1410UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kCanniff_Philip_ProjectTwo1410UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Canniff_Philip_ProjectTwo1410 app using the node.js CLI
