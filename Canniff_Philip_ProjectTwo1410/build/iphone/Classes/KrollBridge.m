...
NSString * Canniff_Philip_ProjectTwo1410$ModuleRequireFormat = @"(function(exports){"
...
@implementation Canniff_Philip_ProjectTwo1410Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_canniff_philip_projecttwo1410);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_canniff_philip_projecttwo1410 gc];
}
...
{
	// create Canniff_Philip_ProjectTwo1410 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Canniff_Philip_ProjectTwo1410" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_canniff_philip_projecttwo1410 = [[Canniff_Philip_ProjectTwo1410Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_canniff_philip_projecttwo1410];
	
	NSString *_canniff_philip_projecttwo1410NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _canniff_philip_projecttwo1410NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_canniff_philip_projecttwo1410 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Canniff_Philip_ProjectTwo1410$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.canniff_philip_projecttwo1410.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.canniff_philip_projecttwo1410.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
